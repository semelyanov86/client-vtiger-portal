...
import { BehaviourStatus, Menus, PlacementStatus } from '../../../stores/menus.ts';
...
export const checkPlacement = ({ placement, breakpoints }: Menus) => {
...
export const checkBehaviour = ({ placement, behaviour, breakpoints }: Menus) => {
  const windowWidth = window.innerWidth;
  // Vertical rules
...
export const isDeeplyDiffPlacementStatus = (a: PlacementStatus, b: PlacementStatus) => {
...
export const isDeeplyDiffBehaviourStatus = (a: BehaviourStatus, b: BehaviourStatus) => {
  if (a.status !== b.status) return true;
  // noinspection RedundantIfStatementJS
